...
export const addFlightTracking = (userId: number, flightId: string, ctx: DurableObjectState<DOProps>) => {
	ctx.storage.sql.exec('INSERT OR IGNORE INTO subscriptions (telegram_id, flight_id) VALUES (?, ?)', userId, flightId)
}

export const removeFlightTracking = (userId: number, flightId: string, ctx: DurableObjectState<DOProps>) => {
	ctx.storage.sql.exec('DELETE FROM subscriptions WHERE telegram_id = ? AND flight_id = ?', userId, flightId)
}

export const untrackFlight = (userId: number, flightId: string, ctx: DurableObjectState<DOProps>) => {
	ctx.storage.sql.exec('DELETE FROM subscriptions WHERE telegram_id = ? AND flight_id = ?', userId, flightId)
}

export const cleanupFlightSubscriptions = (flightId: string, ctx: DurableObjectState<DOProps>) => {
	ctx.storage.sql.exec('DELETE FROM subscriptions WHERE flight_id = ?', flightId)
}

export const clearUserTracking = (userId: number, ctx: DurableObjectState<DOProps>) => {
